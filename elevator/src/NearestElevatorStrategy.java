...
public class NearestElevatorStrategy implements ElevatorSelectionStrategy {
    ...
    public Optional<Elevator> selectElevator(List<Elevator> elevators, Request request) {
        ...
        for (Elevator elevator : elevators) {
            if (isSuitable(elevator, request)) {
                int distance = Math.abs(elevator.getCurrentFloor() - request.getDestination());
    ...
    private boolean isSuitable(Elevator elevator, Request request) {
        ...
        if (elevator.getDirection() == request.getDirection()) {
            if (request.getDirection() == Direction.UP && elevator.getCurrentFloor() <= request.getDestination())
                return true;
            if (request.getDirection() == Direction.DOWN && elevator.getCurrentFloor() >= request.getDestination())
                return true;
        }
        return false;
    }
